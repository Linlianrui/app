    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, '请您微信支付', {
        ...
        beforeClose: (action, instance, done) => {
          /*
            function(action, instance, done)，action 的值为'confirm', 'cancel'或'close'；instance 为 MessageBox 实例，
            可以通过它访问实例上的属性和方法；done 用于关闭 MessageBox 实例
          */
          if (action=='cancel') {
            alert('请联系管理员');
            //关闭定时器
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          //发请求获取用户支付订单状态,code=205支付中，code=200支付完成
